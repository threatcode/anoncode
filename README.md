...
### anoncode
Anoncode will anonymize the entire system under TOR using IPTables. It will also allow you to start and stop i2p as well.

NOTE: DO NOT run this as ```service anoncode $COMMAND```. Run this as ```anoncode $COMMAND```

```bash
Usage:
 anoncode {start|stop|restart|change|status}
 ...
 status - Check if AnonCode is working properly
...
Once the installer is complete, you will be able to use both the anoncode and pandora modules.

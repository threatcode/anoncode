...
dpkg-deb -b  anoncode-deb-src/  anoncode.deb # Build the deb package
dpkg -i  anoncode.deb || (apt-get -f install && dpkg -i  anoncode.deb) # this will automatically install the required packages
